   ...
   public class CSharpGenerator
   {
      ...
      protected virtual void WriteJoinExtension(IndentedTextWriter w, DbSchema schema)
      {
         ...
         foreach (var t in schema.Tables)
         {
            ...
            for(var i = 0; i < fkeys.Length; i++)
            {
               ...
               w.WriteLine("[JoinMethod( Expression = @\"{0}\", {1})]", expression, tableNamesExpression);
      ...
      protected virtual void WriteJoinExtensionChilds(IndentedTextWriter w, DbSchema schema)
      {
         foreach (var t1 in schema.Tables)
         {
            ...
            {
               var fkeys = t.FKeyColumns.Where(c => c.References.Table==t1).OrderBy(k => k.FKey.FKeyConstrantName).ToArray();
               for (var i = 0; i < fkeys.Length; i++)
               {
                  var cols = new List<DbColumnItem>(new[] {fkeys[i]});
                  while (i < fkeys.Length - 1 && fkeys[i].FKey.FKeyConstrantName == fkeys[i + 1].FKey.FKeyConstrantName)
                  ...
                  var y = t1;
                  var oneRelationToThisTable = t.FKeyColumns.Where(c => c.References.Table == y).Select(c => c.FKey.FKeyConstrantName).Count() == 1;
                  ...
                  w.WriteLine("[JoinMethod( Expression = @\"{0}\", NChilds=true, {1})]", expression,tableNamesExpression);
