...
namespace Sql.Parser
{
   ...
   {
      private static readonly HashSet<char> Spaces = new HashSet<char>(new[] { ' ', '\t', '\n', '\r' });
      ...
      public string ReadQuoted()
      {
         var left = Peek();
         char right;
         if (!QuotedHelper.Quotes.TryGetValue(left, out right))
         {
         ...
         throw new SqlParserException(_source, _pos, "Unterminated quoted token: missing " + right);
      }
      public int ReadInt()
      {
         var sb = new StringBuilder();
      ...
      {
         return QuotedHelper.Quotes.ContainsKey(ch);
      }
