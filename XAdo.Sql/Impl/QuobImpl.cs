...
namespace XAdo.Quobs.Impl
{
   public class QuobImpl : IQuob, IAttachable
   {
      ...
      protected QuobImpl(ISqlResource sqlResource, QueryContext context)
         : this(sqlResource)
      {
         Context = context;
      ...
      protected virtual QuobImpl SelfOrNew(QueryContext context, ISqlResource sqlResource = null)
      {
      ...
      {
         var context = Context ?? new QueryContext(SqlResource.Dialect);
      ...
      public virtual IQuob Attach(IXAdoSession session)
      {
         var clone = SelfOrNew(new QueryContext(SqlResource.Dialect) { Session = session });
