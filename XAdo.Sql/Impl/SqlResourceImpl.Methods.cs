...
namespace XAdo.Quobs.Impl
{
   ...
   public partial class SqlResourceImpl
   {
      ...
      public ISqlResource Map<TEntity, TMap>(Expression<Func<TEntity, TMap>> toExpression)
      {
      ...
      public ISqlResource Map(string selectExpression, Type mappedType)
      {
         return _subResourcesCache.GetOrAdd(selectExpression, x =>
         {
            ...
            foreach (var col in columnTuples)
            {
               ...
               {
                  var expression = _filterParser.Parse(col.Item1, mappedType, typeof (object));
      ...
      public SqlGeneratorResult BuildSql(Expression expression, IDictionary<string, object> arguments = null, string parameterPrefix = "xado_", bool noargs = false)
      {
         ...
         {
            var generator = new SqlPredicateGenerator(Dialect, parameterPrefix, noargs);
      ...
      public Func<IDataRecord, object> GetBinder(IXAdoSession session)
      {
         if (session != null)
         {
      ...
      public Type GetEntityType(IXAdoSession session)
      {
         if (_binderCache.Any())
         {
            // any type will do
      ...
      private void EnsureAdoMetaDataBound(IXAdoSession session)
      {
