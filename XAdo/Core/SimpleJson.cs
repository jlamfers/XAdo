...
// MODIFIED by JLA: added support for un-quoted names, and single quoted names/values.

// VERSION:

// NOTE: uncomment the following line to allow unquoted JSON names.
#define ALLOW_UNQUOTED_NAMES

// NOTE: uncomment the following line to allow single quoted JSON names, as well as single quoted JSON string values.
#define ALLOW_SINGLE_QUOTED_STRINGS
...
using System.Text;
using XAdo.Core.SimpleJson.Reflection;

// ReSharper disable LoopCanBeConvertedToQuery
// ReSharper disable RedundantExplicitArrayCreation
// ReSharper disable SuggestUseVarKeywordEvident
namespace XAdo.Core.SimpleJson
{
   /// <summary>
 ...
 static class SimpleJson
   {
      ...
      private const int TOKEN_STRING_SINGLE_QUOTED = 12;
#endif
      ...
      static IDictionary<string, object> ParseObject(char[] json, ref int index, ref bool success)
      {
         ...
         while (!done)
         {
            ...
            else
            {
               ...
               if (token == TOKEN_NONE)
               {
                  name = ParseString(json, ref index, ref success, ':');
               }
#else
               if (token == TOKEN_NONE)
               {
...
#endif
#if ALLOW_SINGLE_QUOTED_STRINGS
               else if (token == TOKEN_STRING_SINGLE_QUOTED)
               {
                  name = ParseString(json, ref index, ref success, '\'');
...
#if ALLOW_SINGLE_QUOTED_STRINGS
            case TOKEN_STRING_SINGLE_QUOTED:
               return ParseString(json, ref index, ref success, '\'');
#endif
            case TOKEN_NUMBER:
      ...
      static string ParseString(char[] json, ref int index, ref bool success, char stopChar)
      {
