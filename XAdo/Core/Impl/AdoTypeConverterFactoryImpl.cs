...
{
    public class AdoTypeConverterFactoryImpl : IAdoTypeConverterFactory
    {
        ...
        public virtual IAdoTypeConverterFactory SetCustomTypeConverter<TSource, TTarget>(Func<TSource, TTarget> converter)
        {
            if (_readonly)
            {
                throw new AdoException("You cannot set additional type converters after you have resolved any custom converter. So make sure bind all needed converters before using any of these.");
            }

            if (Nullable.GetUnderlyingType(typeof(TSource)) != null || Nullable.GetUnderlyingType(typeof(TTarget)) != null)
            {
                throw new AdoException("You must not register nullable type converters. You do not need to. Custom nullable type converions are handled automatically if their corresponding value types have been registered for custom conversion.");
            }

            if (!_customConverters.TryAdd(new ConverterKey(typeof (TSource), typeof(TTarget)), (x => converter((TSource) x))))
            {
                throw new AdoException("Converter already has been registered.");
            }
