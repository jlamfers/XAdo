...
{
    // A xado db session hides connection (and transaction, and sql queue) management, and keeps a connection and its transaction together
   ...
   //  both connection and transaction are created on first need (lazy)
    ...
    public partial class XAdoDbSessionImpl : IXAdoDbSession, IXAdoConnectionProvider, IXAdoSessionInitializer
    {
       private readonly IXAdoConnectionFactory
            _connectionFactory;
       ...
       private Lazy<IDbTransaction> _tr;
        ...
        private readonly IXAdoConnectionQueryManager
            _connectionQueryManager;

       private readonly IXAdoClassBinder _binder;
       ...
       private Lazy<IDbConnection> LazyConnection
           ...
           get
            {
                ...
                {
                    throw new XAdoException("AdoSession has not been initialized yet.");
                }
        ...
        public XAdoDbSessionImpl(IXAdoConnectionFactory connectionFactory,IXAdoConnectionQueryManager connectionQueryManager, XAdoDbContext context,IXAdoClassBinder binder)
        {
        ...
        public virtual IXAdoDbSession Initialize(string connectionStringName, int? commandTimeout = null,
        ...
        public virtual IXAdoDbSession Initialize(string connectionString, string providerName, int? commandTimeout = null,
            bool keepConnectionOpen = false, bool allowUnbindableFetchResults = true,
            bool allowUnbindableMembers = false)
        {
            ...
            if (_cn != null)
            {
                throw new XAdoException("AdoSession has been initialized already");
            }
        ...
        public XAdoDbContext Context { get; private set; }
       ...
       public virtual IEnumerable<dynamic> Query(string sql, object param = null, bool buffered = true,
        ...
        public virtual XAdoMultiResultReader QueryMultiple(string sql, object param = null,CommandType? commandType = null)
        {
            EnsureNotDisposed();
        ...
        public virtual XAdoMultiResultReader QueryMultiple(string sql, IEnumerable<Delegate> factories, object param = null,
          CommandType? commandType = null)
       {
          EnsureNotDisposed();
        ...
        public virtual bool StartSqlBatch()
       {
          ...
          _sqlBatch = _binder.Get<IXAdoSqlBatch>();
          return true;
       }
       ...
       public virtual IXAdoDbSession AddSqlBatchItem(XAdoSqlBatchItem batchItem)
       {
          EnsureNotDisposed();
          var batch = _sqlBatch;
          EnsureSqlBatch();
       ...
       protected virtual void Dispose(bool disposing)
       {
        ...
        private void EnsureNoTransaction()
        {
            if (_tr != null)
            {
                throw new XAdoException("Transaction cannot be started more than once");
            }
        }
        ...
        private void EnsureSqlBatch()
        {
           if (_sqlBatch == null)
           {
              throw new XAdoException("No SQL batch available. SQL cannot be batched");
           }
        }
