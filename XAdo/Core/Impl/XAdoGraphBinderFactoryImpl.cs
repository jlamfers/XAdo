...
{
    public class XAdoGraphBinderFactoryImpl : IXAdoGraphBinderFactory
    {
        private readonly IXAdoDataBinderFactory _binderFactory;

        public XAdoGraphBinderFactoryImpl(IXAdoDataBinderFactory binderFactory)
        {
        ...
        public virtual Func<IDataReader, T> CreateRecordBinder<T, TNext>(IDataRecord record, bool allowUnbindableFetchResults, bool allowUnbindableMembers, ref int nextIndex)
        {
           ...
           if (!allMembers.Any())
           {
              throw new XAdoBindingException("Error with graph binder; No bindable members found for type: "+typeof(T).Name);
           }
            ...
            for (var i = first; i < record.FieldCount; i++)
            {
                ...
                if (set.Remove(name) || !allNextMembers.Contains(name))
                {
                   ...
                   {
                      throw new XAdoBindingException("Error with graph binder; Following column could not be bound: " + name);
                   }
                }
                if (set.Count == 0)
                {
           ...
           {
              throw new XAdoBindingException("Error with graph binder; Following members could not be bound: " + string.Join(", ",set.ToArray()));
           }
            // we finished the datarecord, so we finished type set T as well
            nextIndex = record.FieldCount;
           if (typeof (TNext) != typeof (TVoid))
           {
               throw new XAdoBindingException("Error with graph binder; No columns left to bind.");
           }
