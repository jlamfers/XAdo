...
{
   public partial class XAdoDataReaderManagerImpl : IXAdoDataReaderManager
   {
      #region Helper Types
      private interface IReaderHelper
      {
         IEnumerable Read(IXAdoDataBinderFactory binderFactory, IDataReader reader, bool allowUnbindableFetchResults, bool allowUnbindableMembers);
      }

      private class ReaderHelper<T> : IReaderHelper
      {
         public IEnumerable Read(IXAdoDataBinderFactory binderFactory, IDataReader reader, bool allowUnbindableFetchResults, bool allowUnbindableMembers)
         {
      ...
      {
         IEnumerable ReadAll(XAdoDataReaderManagerImpl parent, IXAdoGraphBinderFactory binderFactory, Func<object, object, object, object, object, object, object, object, object> factory, IDataReader reader, bool allowUnbindableFetchResults, bool allowUnbindableMembers);
      }

      private class GraphReaderHelper<T1, T2, T3, T4, T5, T6, T7, T8, TResult> : IGraphReaderHelper
      {
         public IEnumerable ReadAll(XAdoDataReaderManagerImpl parent, IXAdoGraphBinderFactory binderFactory, Func<object, object, object, object, object, object, object, object, object> factory, IDataReader reader, bool allowUnbindableFetchResults, bool allowUnbindableMembers)
         {
            var next = 0;
            const int index1 = 0;
         ...
         private static T BindTarget<T>(XAdoDataReaderManagerImpl parent, IDataReader reader, int index, Func<IDataReader, T> recordBinder)
         {
      ...
      private readonly IXAdoDataBinderFactory _binderFactory;
      private readonly IXAdoGraphBinderFactory _graphBinderFactory;
      private readonly IXAdoConcreteTypeBuilder _concreteTypeBuilder;

      public XAdoDataReaderManagerImpl(IXAdoDataBinderFactory binderFactory,IXAdoGraphBinderFactory graphBinderFactory, IXAdoConcreteTypeBuilder concreteTypeBuilder)
      {
      ...
      public virtual IEnumerable<dynamic> ReadAll(IDataReader reader)
      {
         ...
         var meta = new XAdoRow.Meta { ColumnNames = columnNames, Index = index, Types = columnTypes };
         while (reader.Read())
         {
