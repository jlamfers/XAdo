...
namespace XAdo.Sql.Core
{
   ...
   public class SelectParser
   {
      ...
      public SelectInfo ParseSelectColumns(string source)
      {
         _pos = 0;
         _source = source;
         ...
         while (!Eof())
         {
            ...
            switch (Peek())
            {
               ...
               case ',':
                  Next();
                  result.Add(new ColumnInfo(expr.ToString().Trim(),alias.ToString().Trim()));
                  expr.Length = 0;
                  alias.Length = 0;
                  readingExpr = true;
                  break;
               default:
                  ...
                  else if (IsLetter())
                  {
                     ...
                     else if (NextIs("FROM"))
                     {
                        ...
                        {
                           SkipSpaces();
                           tablename = tablename + " AS " + ReadUntilSpace();
                        }
                        return new SelectInfo(result.AsReadOnly(),tablename);
                     }
                     else
                     {
