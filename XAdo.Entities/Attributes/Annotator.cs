...
{
   // This Annotator class lets you dynamicly (additionally) annotate types
   public static class Annotator
   {
      ...
      //Note: normally you will not use this method
      public static bool RemoveAnnotation(MemberInfo member, Attribute attribute)
      {
         ...
         IList<Attribute> annotations;
         return Annotations.TryGetValue(identity, out annotations) && annotations.Remove(attribute);
      }
      ...
      private static void CheckUnique(Attribute attribute, IList<Attribute> attributeList)
      {
         ...
         {
            throw new InvalidOperationException(string.Format("AllowMultiple==false => Attribute '{0}' already was added.", attribute.GetType()));
         }
      }
